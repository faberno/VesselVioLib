...
class AnnotationPage(QWidget):
    def __init__(self):
        ...
        pageLayout = QtO.new_layout(self, "H", spacing=0, margins=(0, 20, 40, 20))
        ...
        QtO.add_widgets(
            c1Layout, [50, loadButton, checkSelected, uncheckAll, 0], "Center"
        )
        ...
        QtO.add_widgets(
            c3Layout, [0, 20, addROIs, removeROI, clearROIs, 20, newROI, 0], "Center"
        )
        ...
        QtO.add_widgets(c4Layout, [10, table_label, self.annTable, exportWidget])

        QtO.add_widgets(pageLayout, [column1, column2, column3, column4])
    ...
    def export_ROIs(self):
        ...
        if row_count:
            ...
            file_name = helpers.get_save_file(
...
class AnnotationTree(QTreeWidget):
    ...
    def none(self, event):
        ...
        if len(selected) > 1 and QApplication.keyboardModifiers() & (
            Qt.ShiftModifier | Qt.ControlModifier
        ):
            former_check_states = [
                item.checkState(0) == Qt.Checked for item in selected
            ]
...
class AddROI(QDialog):
    def __init__(self):
        ...
        QtO.add_widgets(nameLayout, [nameLabel, self.nameEdit])
        ...
        QtO.add_widgets(colorLayout, [colorLabel, self.R, self.G, self.B])
        ...
        QtO.add_widgets(idLayout, [idLabel, self.idBox, 0])
        ...
        QtO.add_widgets(buttonLayout, [0, addButton, cancelButton])

        QtO.add_widgets(dialogLayout, [nameLayout, colorLayout, idLayout, buttonLayout])
...
class LoadTreeFile(QDialog):
    def __init__(self):
        ...
        QtO.add_widgets(boxLayout, [self.defaultList, 5, loadLayout], "Right")
        ...
        advanced_help = (
            "<b>Case Sensitive</b> processing options for JSON file loading."
        )
