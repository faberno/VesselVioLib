...
def load_csv_vertices(
    filename, delimiter, graph_type, a_key, resolution, Visualize=False
):
...
def load_csv_edges(
    g, filename, delimiter, graph_type, a_key, resolution, Visualize=False
):
    data = pandas.read_csv(filename, delimiter=delimiter)
...
def graph_loading_dock(filename, g_options, resolution, Visualize=False):
    if g_options.file_format != "csv":
        ...
        if g_options.graph_type == "Centerlines":
            g = build_vert_graph(
                raw_g, g_options.a_key, resolution, Visualize=Visualize
            )
