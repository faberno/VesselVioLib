...
def feature_extraction(
    ...
    if centerline_smoothing:
        ...
        coords = seg_interpolate(coords, r_avg / MIN_RESOLUTION)
...
def small_seg_path(
    ...
    if pruning:
        segment_length = feature_extraction(
            g,
            point_list,
            resolution,
...
def large_seg_filter(g, segment, resolution, centerline_smoothing=True):
    ...
    if len(endpoints) == 2:
        ...
        point_list = g.get_shortest_paths(
    ...
    # Interpolate points for a smoothed centerline.
    if centerline_smoothing:
        point_coords = seg_interpolate(
            point_coords, np.mean(g.vs[point_list]["v_radius"]) / MIN_RESOLUTION
        )
...
def vgraph_analysis(
    ...
    workers = cpu_count()
    if unix_check() and seg_count > workers:
        results = multiprocessing_input(
            segment_feature_extraction, seg_count, workers, sublist=True
        )
    ...
    if unix_check() and seg_count > workers:
        results = multiprocessing_input(
            branch_segment_feature_extraction, seg_count, workers, sublist=True
        )
        for result in results:
            features.extend(result[0])
            edges.extend(result[1])
...
def feature_input(
    ...
    if image_dim == 3:
        ...
        avg_volume_or_PAF = (
            np.mean(volumes_or_PAFs) if image_shape else "NA"
        )  # For 2D graphs
