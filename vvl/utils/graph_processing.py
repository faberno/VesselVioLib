...
from vvl.utils.feature_processing import small_seg_path, large_seg_path, large_seg_filter
from vvl.utils.volume_processing import absolute_points
from vvl.utils.helpers import unix_check, multiprocessing_input
...
def class2and3_processing():
    ...
    workers = cpu_count()
    if unix_check() and clique_count > workers:
...
def class1_processing():
    ...
    clique_count = len(cliques)
    if unix_check() and clique_count > workers:
        edges_togo = multiprocessing_input(class1_filter, clique_count, workers)
    else:
...
def create_graph(
    ...
    g.vs["v_coords"] = absolute_points(points, point_minima)
